...
/// Error state when an error occurs while attempting to deserialize a RTMP chunk
#[derive(Debug)]
...
pub enum ChunkDeserializationErrorKind {
    ...
    /// from the previously received chunk on that same stream id.  This error occurs when a
    /// non-0 chunk is received on a stream that has not received a type 0 chunk yet.
    #[fail(display = "Received chunk with non-zero chunk type on csid {} prior to receiving a type 0 chunk", csid)]
    NoPreviousChunkOnStream {
        csid: u32
    },

    /// The max chunk size does not allow chunk sizes more than 2147483647 (since it's encoded in only
    ...
    #[fail(display = "Requested an invalid max chunk size of {}.  The largest chunk size possible is 2147483647", chunk_size)]
    InvalidMaxChunkSize {
        chunk_size: usize
    },
...
impl From<io::Error> for ChunkDeserializationError {
    fn from(error: io::Error) -> Self {
        ChunkDeserializationError { kind: ChunkDeserializationErrorKind::Io(error) }
    }
}